...
| Upload phase does nothing / "already on the device; skipping" | The ISO is already present (a previous run, or a manual upload) | Expected and idempotent. To force a fresh upload, remove it: `tmsh delete sys software image <iso>` |
| Playbook run warns `ansible.netcommon does not support Ansible version 2.14.x` | Running under an unsupported ansible-core (system Python 3.9) | Use ansible-core 2.16+ in a Python 3.10+ venv — see the README requirements |
